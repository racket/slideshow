...
(module slideshow mzscheme
  ...
  (define-values/invoke-unit/sig ((open core^) (unit config : cmdline^))
    (compound-unit/sig
     (import)
     (link [CONFIG : cmdline^ (cmdline@)]
	   [VIEWER : viewer^ ((unit/sig viewer^
				...
				(define (set-init-page! p)
				  (set! current-page p))
				(set-init-page! cmdline:init-page)
				...
				(set-page-numbers-visible! cmdline:show-page-numbers?)
	   ...
	   [CORE : core^ (core@ (CONFIG : config^)
				VIEWER)])
     (export (open CORE) (unit CONFIG config))))
  ...
  ;; Also, we re-export some of config^
  ...
  (provide most-recent-slide retract-most-recent-slide re-slide start-at-recent-slide
	   ...
	   (rename config:printing? printing?)
	   (rename config:condense? condense?)
	   skip-slides
  ...
  (define go@
	...
	(send background-f show #t))
      ...
      (define f (new talk-frame%
		     ...
		     [close-bg? #t]
                     [label (if config:content
                                (format "~a: slideshow" (file-name-from-path config:content))
                                "Slideshow")]
                     [x (- screen-left-inset)] [y (- screen-top-inset)]
                     [width (inexact->exact (floor config:actual-screen-w))]
                     [height (inexact->exact (floor config:actual-screen-h))]
  ...
  (when config:content
    (load-content (string->path config:content)))
